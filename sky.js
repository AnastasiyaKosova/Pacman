document.addEventListener("DOMContentLoaded", () => {
    ...
    canvas.addEventListener("mousemove", (e) => {
        ...
        if (foundIndex >= 0) {
            ...
            let cardLeft = e.clientX + 15;
            let cardTop = e.clientY + 15;

            // Adjust left position to keep card within viewport
            if (cardLeft + cardWidth > windowWidth) {
                cardLeft = windowWidth - cardWidth - 15;
            }
            if (cardLeft < 0) {
                cardLeft = 10; // Minimum margin from left
            }

            // Adjust top position to keep card within viewport
            if (cardTop + cardHeight > windowHeight) {
                cardTop = windowHeight - cardHeight - 15;
            }
            if (cardTop < 0) {
                cardTop = 10; // Minimum margin from top
            }
